...
// has all the device register declarations
...
void PortF_Init(void){ volatile unsigned long delay;
  ...
  GPIO_PORTF_DIR_R &= ~0x10;        //   PF4 input
	GPIO_PORTF_DIR_R |= 0x0A;         //   PF2 output
  GPIO_PORTF_AFSEL_R &= ~0x1A;      // Disable alt funct on PF4 and PF2
  GPIO_PORTF_PUR_R |= 0x10;         // Enable pull-up on PF4
  GPIO_PORTF_DEN_R |= 0x1A;         // Enable digital I/O on PF4 and PF2
}
